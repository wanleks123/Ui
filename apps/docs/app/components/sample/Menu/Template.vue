<template>
    <DocsCard>
        <UMenu :model="items" class="w-full md:w-60">
            <template #start>
                <span class="inline-flex items-center gap-1 px-2 py-2">
                    ...
                    <span class="text-xl font-semibold ml-2">PRIME<span class="text-primary">APP</span></span>
                </span>
            </template>

            <template #submenulabel="{ item }">
                <span class="text-primary font-bold px-2">{{ item.label }}</span>
            </template>

            <template #item="{ item, props }">
                <a v-ripple class="flex items-center px-3 py-2 cursor-pointer" v-bind="props.action">
                    <Icon :name="item.icon" style="width: 18px; height: 18px;" class="flex-shrink-0" />
                    <span class="ml-2">{{ item.label }}</span>
                    
                    <UBadge v-if="item.badge" class="ml-auto " >{{ item.badge }}</UBadge>
                    
                    <span v-if="item.shortcut" class="ml-auto border border-surface rounded bg-emphasis text-muted-color text-[10px] px-1 py-0.5">{{ item.shortcut }}</span>
                </a>
            </template>

            <template #end>
                <button v-ripple class="relative overflow-hidden w-full border-0 bg-transparent flex items-center p-2 pl-4 hover:bg-surface-100 dark:hover:bg-surface-800 cursor-pointer transition-colors duration-200">
                    <UAvatar image="https://primefaces.org/cdn/primevue/images/avatar/amyelsner.png" class="mr-2" shape="circle" />
...
const items = ref([
    { separator: true },
    {
        label: 'Documents',
        items: [
            { label: 'New', icon: 'ph:plus', shortcut: '⌘+N' },
            { label: 'Search', icon: 'ph:magnifying-glass', shortcut: '⌘+S' }
        ]
    },
    {
        label: 'Profile',
        items: [
            { label: 'Settings', icon: 'ph:gear', shortcut: '⌘+O' },
            { label: 'Messages', icon: 'ph:chat-circle-dots', badge: 2 },
            { label: 'Logout', icon: 'ph:sign-out', shortcut: '⌘+Q' }
        ]
    },
    { separator: true }
]);
