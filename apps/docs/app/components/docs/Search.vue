        ...
        <TransitionRoot :show="isOpen" as="template">
            <Dialog @close="close" class="fixed inset-0 z-50">
                <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0" enter-to="opacity-100" leave="duration-200 ease-in" leave-from="opacity-100" leave-to="opacity-0">
                    <div class="fixed inset-0 bg-surface-900/50 backdrop-blur-sm" />
                </TransitionChild>
                ...
                <div class="fixed inset-0 overflow-y-auto px-4 py-4 sm:px-6 sm:py-20 md:py-32 lg:px-8 lg:py-[15vh]">
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100" leave="duration-200 ease-in" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
                        <DialogPanel class="mx-auto transform-gpu overflow-hidden rounded-xl bg-white shadow-xl sm:max-w-xl dark:bg-surface-800 dark:ring-1 dark:ring-surface-700">
                            <div v-bind="autocomplete.getRootProps({})">
                                <form v-bind="autocomplete.getFormProps({ inputElement: null })">
                                    <div class="group relative flex h-12">
                                        <Icon name="ph:magnifying-glass" class="pointer-events-none absolute top-0 left-4 h-full w-5 text-surface-400 dark:text-surface-500" />
                                        <input
                                            class="flex-auto appearance-none bg-transparent pl-12 text-surface-900 outline-none placeholder:text-surface-400 focus:w-full focus:flex-none sm:text-sm dark:text-white"
                                        ...
                                        <div v-if="autocompleteState.status === 'stalled'" class="absolute inset-y-0 right-3 flex items-center">
                                            <Icon name="ph:circle-notch" class="h-6 w-6 animate-spin text-surface-400 dark:text-surface-500" />
                                        </div>
                                    </div>

                                    <div v-if="autocompleteState.isOpen" class="border-t border-surface-200 bg-white px-2 py-3 dark:border-surface-400/10 dark:bg-surface-800">
                                        <div v-if="autocompleteState.collections[0]?.items.length === 0" class="px-4 py-8 text-center text-sm text-surface-700 dark:text-surface-400">
                                        ...
                                        <ul v-else v-bind="autocomplete.getListProps()">
                                            <li
                                                ...
                                                :key="result.url"
                                                class="group block cursor-default rounded-lg px-3 py-2 aria-selected:bg-surface-100 dark:aria-selected:bg-surface-700/30"
                                                v-bind="autocomplete.getItemProps({ item: result, source: autocompleteState.collections[0].source })"
                                            >
                                                <div class="text-sm text-surface-700 group-aria-selected:text-sky-600 dark:text-surface-300 dark:group-aria-selected:text-sky-400">
                                                    <HighlightQuery :text="result.title" :query="autocompleteState.query" />
                                                </div>
                                                <div class="mt-0.5 truncate text-xs whitespace-nowrap text-surface-500 dark:text-surface-400">
                                                    <template v-for="(item, idx) in getHierarchy(result)" :key="idx">
                                                        <HighlightQuery :text="item" :query="autocompleteState.query" />
                                                        <span v-if="idx < getHierarchy(result).length - 1" class="mx-2 text-surface-300 dark:text-surface-700">/</span>
                                                    </template>
                                                </div>
                                            </li>
                                        </ul>
